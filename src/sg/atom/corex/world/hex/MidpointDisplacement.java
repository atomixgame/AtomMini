package sg.atom.corex.world.hex;
...
public class MidpointDisplacement {
    ...
    public int[][] getMap(int n, int wmult, int hmult) {
        ...
        int power = (int) FastMath.pow(2, n);
        ...
        while (step > 0) {
            ...
            for (int x = step; x < width; x += 2 * step) {
                for (int y = step; y < height; y += 2 * step) {
                    ...
                    map[x][y] = sum / 4 + nextRandomFloat(-h, h);
                }
            }
            ...
            for (int x = 0; x < width; x += step) {
                for (int y = step * (1 - (x / step) % 2); y < height; y += 2 * step) {
                    ...
                    if (count > 0) {
                        map[x][y] = sum / count + nextRandomFloat(-h, h);
                    } else {
                        map[x][y] = 0;
                    }
                }
    ...
    private float nextRandomFloat(float minV, float maxV) {
        return FastMath.nextRandomFloat() * minV + (maxV - minV);
    }

    private float nextRandomFloat(float value) {
        return FastMath.nextRandomFloat() * value;
    }
}
